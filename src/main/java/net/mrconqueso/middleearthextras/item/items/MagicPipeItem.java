...
public class MagicPipeItem extends PipeItem {
    private final int USAGE_TIME = 60;
    private boolean smoking;
    private int usesPerLeaf;

    public MagicPipeItem(Settings settings, int amountOfUses) {
        super(settings, amountOfUses);
    ...
    public void spawnSmoke(int remainingUseTicks, LivingEntity user, World world){
        ...
        SmokeBoatProjectileEntity smoke = new SmokeBoatProjectileEntity(ModEntities.SMOKE_BOAT_PROJECTILE,
                world,
                user);

        Vec3d lookDir = user.getRotationVec(1.0F).normalize();
        
        double spawnX = user.getX() + lookDir.x * 0.5;
        double spawnY = user.getEyeY() - 0.1 + lookDir.y * 0.5;
        double spawnZ = user.getZ() + lookDir.z * 0.5;

        float yaw = (float) Math.toDegrees(Math.atan2(-lookDir.x, lookDir.z));
        float pitch = (float) Math.toDegrees(-Math.atan2(lookDir.y, Math.sqrt(lookDir.x * lookDir.x + lookDir.z * lookDir.z)));

        smoke.refreshPositionAndAngles(spawnX,
                spawnY,
                spawnZ,
                yaw,
                pitch);

        Vec3d dir = lookDir.multiply(f);
